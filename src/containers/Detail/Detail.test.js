...
describe('Detail container', () => {
  ...
  it('should render correctly', () => {
    ...
    expect(SampleDescription.innerHTML).toEqual('Descrição')
    expect(SampleTitleSize.innerHTML).toEqual('Dimensões')
    expect(SampleTitleVoltage.innerHTML).toEqual('Tensão')
    expect(SampleSize.innerHTML).toEqual(`AL: ${productMock.height} cm x L: ${productMock.width} cm x P: ${productMock.deep} cm - Peso: ${productMock.weight} kg`)
    expect(SampleImgSrc.getAttribute('src')).toEqual(productMock.image)
    expect(SampleEquimentCode.innerHTML).toEqual(productMock.name)
    expect(SampleEquimentVoltage.innerHTML).toEqual(productMock.voltage)
